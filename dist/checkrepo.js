...
function fetchGitHubRepositoryInfo() {
    return __awaiter(this, void 0, void 0, function () {
        var owner, name, query, response, data, lastCommitDate, currentDate, timeDifference, daysSinceLastCommit, readmeText, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    owner = 'thenativeweb';
                    name = 'timer2';
                    query = fs_1.default.readFileSync('queries.txt', 'utf-8');
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                ...
                case 2:
                    response = _a.sent();
                    data = response.data.data.repository;
                    lastCommitDate = new Date(data.defaultBranchRef.target.history.edges[0].node.committedDate);
                    currentDate = new Date();
                    timeDifference = currentDate.getTime() - lastCommitDate.getTime();
                    daysSinceLastCommit = Math.floor(timeDifference / (1000 * 60 * 60 * 24));
                    readmeText = data.object.text;
                    // You can use regular expressions to extract specific information from the README.md file here.
                    console.log("Last Commit Date: ".concat(lastCommitDate.toISOString()));
                    console.log("Days Since Last Commit: ".concat(daysSinceLastCommit));
                    console.log("README.md Text:\n".concat(readmeText));
                    return [3 /*break*/, 4];
                case 3:
                    error_1 = _a.sent();
                    console.error('Error fetching repository info:', error_1);
                    return [3 /*break*/, 4];
